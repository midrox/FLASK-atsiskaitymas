...
def repair():
    ...
    if form.validate_on_submit():
        ...
        flash('Successful registration', 'succes')
        return redirect(url_for('home'))
    return render_template('repair.html', form=form, current_user=current_user)  

@app.route('/all_cars')
@login_required
def all_cars():
    page = request.args.get('page', 1, type=int)
    visi_automobiliai = Repair.query.filter_by(car_id=current_user.id).paginate(page=page, per_page=5)
